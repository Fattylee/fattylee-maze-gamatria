...
export class Maze {
  constructor(size, rows, columns, maze, ctx) {
    this.size = size;
  ...
  setup() {
    ...
    GlobalVar.current = this.grid[0][0];
    this.grid[this.rows - 1][this.columns - 1].goal = true;
  }

  // Draw the canvas by setting the size and placing the cells in the grid array on the canvas.
  draw() {
    this.maze.width = this.size;
    this.maze.height = this.size;
    this.maze.style.background = "black";
    // Set the first cell as visited
    GlobalVar.current.visited = true;
    ...
    if (next) {
      ...
      GlobalVar.current.removeWalls(GlobalVar.current, next);
